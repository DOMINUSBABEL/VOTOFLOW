...
export const aggregateByLocation = (records: VoteRecord[]): LocationData[] => {
  ...
  for (const record of records) {
    ...
    if (baseCoords) {
      ...
      // Aggregate Parties
      if (!entry.parties[record.partyName]) {
        entry.parties[record.partyName] = 0;
      }
      entry.parties[record.partyName] += record.votes;
    }
  }

  // Determine Winning Party for each location
  const result = Array.from(map.values()).map(loc => {
    let winningParty = "";
    let maxPartyVotes = -1;
    Object.entries(loc.parties).forEach(([party, count]) => {
      if (count > maxPartyVotes) {
        maxPartyVotes = count;
        winningParty = party;
      }
    });
    return { ...loc, winningParty };
  });
